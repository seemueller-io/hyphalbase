...
describe('Chunker', () => {
  ...
  it('should chunk an array of objects with content property', () => {
    const input = [{ content: 'First part.' }, { content: 'Second part.' }];
  ...
  it('should check if text is within token limit', () => {
    ...
    const longText =
      'This is a longer text that might exceed a very small token limit depending on the tokenizer used.';
  ...
  it('should generate chunks using generator function', () => {
    const input =
      'This is a test document for the generator function. It has multiple sentences to process.';
    const generator = chunkDocumentGenerator(input, {
      chunkSize: 10,
      overlap: 2,
    });
